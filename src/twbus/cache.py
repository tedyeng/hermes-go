...
class SQLiteCache:
    def __init__(self, expiration_seconds: int = 3600):  # Default 1 hour
        self.expiration_seconds = expiration_seconds
        cache_dir = Path(user_cache_dir("twbus"))
    ...
    def set(self, key: str, value: Any, custom_expiry: Optional[int] = None):
...
# Globally shared cache instance for twbus
cache = SQLiteCache()

def get_cached_token() -> Optional[str]:
    """Helper to get a valid cached TDX Token."""
    token_data = cache.get("tdx_token")
    if token_data and isinstance(token_data, dict):
        # Double check expires_at
        if token_data.get("expires_at", 0) > time.time():
            return token_data.get("access_token")
    return None

def set_cached_token(token_data: dict):
    """Helper to cache TDX Token data."""
    # Cache duration is calculated from expires_at minus current time
    expires_at = token_data.get("expires_at", 0)
    ttl = int(expires_at - time.time())
    if ttl > 0:
        cache.set("tdx_token", token_data, custom_expiry=ttl)
